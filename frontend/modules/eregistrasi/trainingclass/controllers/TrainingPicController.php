...
use backend\models\TrainingPicSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;

/**
 * TrainingPicController implements the CRUD actions for TrainingPic model.
 */
class TrainingPicController extends Controller
{
    ...
    /**
     * Lists all TrainingPic models.
     * @return mixed
     */
    public function actionIndex($tb_training_id)
    {
        $searchModel = new TrainingPicSearch();
    ...
    /**
     * Displays a single TrainingPic model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new TrainingPic model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate()
    {
        $model = new TrainingPic();
    ...
    /**
     * Updates an existing TrainingPic model.
    ...
    /**
     * Deletes an existing TrainingPic model.
    ...
    /**
     * Finds the TrainingPic model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return TrainingPic the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TrainingPic::findOne($id)) !== null) {
            return $model;
        } else {
	...
	public function actionEditable() {
		$model = new TrainingPic; // your model can be loaded here
		...
		if (isset($_POST['hasEditable'])) {
			...
			if ($model->load($_POST)) {
				...
				$name=key($_POST['TrainingPic'][$_POST['editableIndex']]);
				$value=$_POST['TrainingPic'][$_POST['editableIndex']][$name];
				$model2->$name = $value ;
				$model2->save();
	...
	public function actionOpenTbs($filetype='docx'){
		...
		try {
			...
			$OpenTBS->VarRef['modelName']= "TrainingPic";
			$data1[]['col0'] = 'id';			
			$data1[]['col1'] = 'tb_training_class_id';			
			$data1[]['col2'] = 'tb_user_id';			
			$data1[]['col3'] = 'type';			
	
			$OpenTBS->MergeBlock('a', $data1);			
			$data2 = [];
			foreach($dataProvider->getModels() as $trainingpic){
				$data2[] = [
					'col0'=>$trainingpic->id,
					'col1'=>$trainingpic->tb_training_class_id,
					'col2'=>$trainingpic->tb_user_id,
					'col3'=>$trainingpic->type,
				];
			}
			...
			if($template=='yes'){
				...
				if(in_array($filetype,['xlsx','xls'])){
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('A1', 'Tabel TrainingPic');
					$idx=2; // line 2
					foreach($dataProvider->getModels() as $trainingpic){
						$objPHPExcel->getActiveSheet()->setCellValue('A'.$idx, $trainingpic->id)
													  ->setCellValue('B'.$idx, $trainingpic->tb_training_class_id)
													  ->setCellValue('C'.$idx, $trainingpic->tb_user_id)
													  ->setCellValue('D'.$idx, $trainingpic->type);
						$idx++;
					}		
			...
			if(!empty($importFile)){
				...
				if(in_array($ext,$fileTypes)){
					...
					while(!empty($sheetData[$baseRow]['A'])){
						...
						$tb_user_id=  $sheetData[$baseRow]['C'];
						...
						$model2=new TrainingPic;
						//$model2->id=  $id;
						$model2->tb_training_class_id=  $tb_training_class_id;
						$model2->tb_user_id=  $tb_user_id;
						$model2->type=  $type;
