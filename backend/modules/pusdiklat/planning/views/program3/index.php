			...
			[
				...
				'value' => function ($data) {
					$countSubject = \backend\models\ProgramDocument::find()
								...
								->count();
					if($countSubject>0){
						return Html::a($countSubject, ['program-document3/index','tb_program_id'=>$data->id], ['class' => 'label label-primary']);
					}
					else{
						return Html::a('+', ['program-document3/index','tb_program_id'=>$data->id], ['class' => 'label label-primary']);
					}
				}
			],
			[
				...
				'value' => function ($data) {
					$countRevision = \backend\models\ProgramHistory::find()
								...
								->count()-1;
					if($countRevision>0){
						return Html::a($countRevision.'x', ['program-history3/index','tb_program_id'=>$data->id], ['class' => 'label label-danger']);
					}
					else{
						return '-';
					}
			...
			[
				...
				'value' => function ($data) {
					$countSubject = \backend\models\ProgramSubject::find()
								...
								->count();
					if($countSubject>0){
						return Html::a($countSubject, ['program-subject3/index','tb_program_id'=>$data->id], ['class' => 'label label-success']);
					}
					else{
						return Html::a('+', ['program-subject3/index','tb_program_id'=>$data->id], ['class' => 'label label-success']);
					}
				}
			],
			[
			...
			'before'=>
				...
				Select2::widget([
					...
					'options' => [
						...
						'onchange'=>'
							$.pjax.reload({
								url: "'.\yii\helpers\Url::to(['/'.$controller->module->uniqueId.'/program3/index']).'?status="+$(this).val(), 
