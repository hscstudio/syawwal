...
<div class="training-class-index">
    ...
    <?= GridView::widget([
        ...
        'columns' => [
			...
			[
				...
				'value' => function ($model) use ($sbu){	
					return '<span class="label label-default" id="tarif_'.$model->id.'">'.number_format($sbu->value).'</span>';
				}
			],
			[
				'label' => 'Frek',
			...
			/*[
				...
				'value' => function ($model) use ($sbu){	
					...
					return '<span class="label label-default" id="pph_'.$model->id.'">'.($total).'</span>';
				}
			],*/
			[
				'label' => 'Total',
				...
				'value' => function ($model)use($sbu){
					...
					$total = $sbu->value; //- $total_pph; transport tidak kena pajak
					return '<span class="label label-default" id="total_'.$model->id.'">'.number_format($total).'</span>';
				}
			],
		...
		'panel' => [
			'heading'=>'<h3 class="panel-title"><i class="fa fa-fw fa-globe"></i> Honor Transport</h3>',
			'before'=>
