...
use backend\models\TrainingClassSubjectSearch;
...
class TrainingClassSubjectController extends Controller
{
     ...
     * @return mixed
     */
    public function actionCreate($tb_training_class_id)
    {
        $trainingClass=\backend\models\TrainingClass::findOne($tb_training_class_id);
        $program = $trainingClass->training->tb_program_id;
        $program_revision = $trainingClass->training->tb_program_revision;
        ...
        foreach($programSubjects as $programSubject){
            ...
            if($subjectCount==0){
                ...
                $model->tb_program_subject_id=$programSubject->tb_program_subject_id;
                $model->status=1;
                if($model->save()){
                    $created++;
                }
        ...
        return $this->redirect(['index', 'tb_training_class_id' => $tb_training_class_id]);
    }
