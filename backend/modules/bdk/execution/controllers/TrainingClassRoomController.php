...
/**
 * TrainingClassRoomController implements the CRUD actions for TrainingClassRoom model.
 */
class TrainingClassRoomController extends Controller
{
    ...
    /**
     * Lists all TrainingClassRoom models.
     * @return mixed
     */
    ...
    /**
     * Displays a single TrainingClassRoom model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new TrainingClassRoom model.
    ...
    /**
     * Updates an existing TrainingClassRoom model.
    ...
    /**
     * Deletes an existing TrainingClassRoom model.
    ...
    /**
     * Finds the TrainingClassRoom model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return TrainingClassRoom the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = TrainingClassRoom::findOne($id)) !== null) {
            return $model;
        } else {
	...
	public function actionOpenTbs($filetype='docx'){
		...
		try {
			...
			$OpenTBS->VarRef['modelName']= "TrainingClassRoom";
			$data1[]['col0'] = 'id';			
			...
			$data2 = [];
			foreach($dataProvider->getModels() as $trainingclassroom){
				$data2[] = [
					'col0'=>$trainingclassroom->id,
					'col1'=>$trainingclassroom->tb_training_id,
					'col2'=>$trainingclassroom->class,
					'col3'=>$trainingclassroom->tb_room_id,
				];
			}
			...
			if($template=='yes'){
				...
				if(in_array($filetype,['xlsx','xls'])){
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('A1', 'Tabel TrainingClassRoom');
					$idx=2; // line 2
					foreach($dataProvider->getModels() as $trainingclassroom){
						$objPHPExcel->getActiveSheet()->setCellValue('A'.$idx, $trainingclassroom->id)
													  ->setCellValue('B'.$idx, $trainingclassroom->tb_training_id)
													  ->setCellValue('C'.$idx, $trainingclassroom->class)
													  ->setCellValue('D'.$idx, $trainingclassroom->tb_room_id);
						$idx++;
					}		
