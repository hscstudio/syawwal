...
class SbuController extends Controller
{
    ...
    {		
		...
		if($status!='all'){
			$queryParams['SbuSearch']=[
				'ref_sbu_id'=>(int)Yii::$app->user->identity->employee->ref_satker_id,
				'status'=>$status,
			];
		}
		else{
	...
	public function actionEditable() {
		$model = new Sbu; // your model can be loaded here
		...
		if (isset($_POST['hasEditable'])) {
			...
			if ($model->load($_POST)) {
				...
				$name=key($_POST['Sbu'][$_POST['editableIndex']]);
				$value=$_POST['Sbu'][$_POST['editableIndex']][$name];
				$model2->$name = $value ;
				$model2->save();
	...
	public function actionOpenTbs($filetype='docx'){
		...
		try {
			...
			$data2 = [];
			foreach($dataProvider->getModels() as $sbu){
				$data2[] = [
					'col0'=>$sbu->id,
					'col1'=>$sbu->name,
					'col2'=>$sbu->value,
			...
			if($template=='yes'){
				...
				if(in_array($filetype,['xlsx','xls'])){
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('A1', 'Tabel Sbu');
					$idx=2; // line 2
					foreach($dataProvider->getModels() as $sbu){
						$objPHPExcel->getActiveSheet()->setCellValue('A'.$idx, $sbu->id)
													  ->setCellValue('B'.$idx, $sbu->name)
													  ->setCellValue('C'.$idx, $sbu->value)
													  ->setCellValue('D'.$idx, $sbu->status);
						$idx++;
					}		
