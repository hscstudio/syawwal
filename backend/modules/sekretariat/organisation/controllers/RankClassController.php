...
class RankClassController extends Controller
{
	...
	public function actionEditable() {
		$model = new RankClass; // your model can be loaded here
		...
		if (isset($_POST['hasEditable'])) {
			...
			if ($model->load($_POST)) {
				...
				$name=key($_POST['RankClass'][$_POST['editableIndex']]);
				$value=$_POST['RankClass'][$_POST['editableIndex']][$name];
				$model2->$name = $value ;
				$model2->save();
	...
	public function actionOpenTbs($filetype='docx'){
		...
		try {
			...
			$OpenTBS->VarRef['modelName']= "RankClass";
			$data1[]['col0'] = 'id';			
			$data1[]['col1'] = 'name';			
			$data1[]['col2'] = 'status';			
			$data1[]['col3'] = 'created';			
			...
			$data2 = [];
			foreach($dataProvider->getModels() as $rankclass){
				$data2[] = [
					'col0'=>$rankclass->id,
					'col1'=>$rankclass->name,
					'col2'=>$rankclass->status,
					'col3'=>$rankclass->created,
				];
			}
			...
			if($template=='yes'){
				...
				if(in_array($filetype,['xlsx','xls'])){
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('A1', 'Tabel RankClass');
					$idx=2; // line 2
					foreach($dataProvider->getModels() as $rankclass){
						$objPHPExcel->getActiveSheet()->setCellValue('A'.$idx, $rankclass->id)
													  ->setCellValue('B'.$idx, $rankclass->name)
													  ->setCellValue('C'.$idx, $rankclass->status)
													  ->setCellValue('D'.$idx, $rankclass->created);
						$idx++;
					}		
