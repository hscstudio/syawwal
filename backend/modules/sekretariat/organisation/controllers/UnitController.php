...
class UnitController extends Controller
{
    ...
    public function actionIndex($status=1)
    {
		...
		if($status!='all'){
			$queryParams['UnitSearch']=[
				'ref_unit_id'=>(int)Yii::$app->user->identity->employee->ref_satker_id,
				'status'=>$status,
			];
		}
		else{
	...
	public function actionEditable() {
		$model = new Unit; // your model can be loaded here
		...
		if (isset($_POST['hasEditable'])) {
			...
			if ($model->load($_POST)) {
				...
				$name=key($_POST['Unit'][$_POST['editableIndex']]);
				$value=$_POST['Unit'][$_POST['editableIndex']][$name];
				$model2->$name = $value ;
				$model2->save();
			...
			if($template=='yes'){
				...
				if(in_array($filetype,['xlsx','xls'])){
					...
					$objPHPExcel->setActiveSheetIndex(0)
								->setCellValue('A1', 'Tabel Unit');
					$idx=2; // line 2
					foreach($dataProvider->getModels() as $unit){
						$objPHPExcel->getActiveSheet()->setCellValue('A'.$idx, $unit->id)
													  ->setCellValue('B'.$idx, $unit->name)
													  ->setCellValue('C'.$idx, $unit->shortname)
													  ->setCellValue('D'.$idx, $unit->status);
						$idx++;
					}		
